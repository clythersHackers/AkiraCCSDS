...
static const uint8_t gamepad_report_desc[] = {    0x05, 0x01, /* Usage Page (Generic Desktop) */
...
static ssize_t read_ms_report(struct bt_conn *conn,
                              ...
                              void *buf, uint16_t len, uint16_t offset)
{
    return bt_gatt_attr_read(conn, attr, buf, len, offset, &current_mouse, sizeof(current_mouse));
}
...
static ssize_t read_cs_report(struct bt_conn *conn,
                              ...
                              void *buf, uint16_t len, uint16_t offset)
{
    return bt_gatt_attr_read(conn, attr, buf, len, offset, &current_consumer, sizeof(current_consumer));
}
                                    ...
                                    uint16_t offset, uint8_t flags)
{
    ...
    LOG_DBG("HID: Protocol Mode write 0x%02x (ignored — staying in Report Protocol)", requested);
    return len;
}
...
BT_GATT_SERVICE_DEFINE(hids_svc,
                       ...
                       BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_INFO, BT_GATT_CHRC_READ,
                                              BT_GATT_PERM_READ_ENCRYPT, read_hid_info, NULL, (void *)hid_info),
                       BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_REPORT_MAP, BT_GATT_CHRC_READ,
                                              BT_GATT_PERM_READ_ENCRYPT, read_report_map_cb, NULL, NULL),

                       /* Keyboard report (ID = 1) */
                       BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_REPORT, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                                              BT_GATT_PERM_READ_ENCRYPT, read_kb_report, NULL, &current_keyboard),
                       BT_GATT_DESCRIPTOR(BT_UUID_HIDS_REPORT_REF, BT_GATT_PERM_READ_ENCRYPT, read_report_ref, NULL, (void *)kb_report_ref),
                       BT_GATT_CCC(NULL, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),

                       /* Gamepad report (ID = 2) */
                       BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_REPORT, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                                              BT_GATT_PERM_READ_ENCRYPT, read_gp_report, NULL, &current_gamepad),
                       BT_GATT_DESCRIPTOR(BT_UUID_HIDS_REPORT_REF, BT_GATT_PERM_READ_ENCRYPT, read_report_ref, NULL, (void *)gp_report_ref),
                       BT_GATT_CCC(NULL, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),

                       /* Mouse report (ID = 3) */
                       BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_REPORT, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                                              BT_GATT_PERM_READ_ENCRYPT, read_ms_report, NULL, &current_mouse),
                       BT_GATT_DESCRIPTOR(BT_UUID_HIDS_REPORT_REF, BT_GATT_PERM_READ_ENCRYPT, read_report_ref, NULL, (void *)ms_report_ref),
                       BT_GATT_CCC(NULL, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),

                       /* Consumer / Media keys report (ID = 4) */
                       BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_REPORT, BT_GATT_CHRC_READ | BT_GATT_CHRC_NOTIFY,
                                              BT_GATT_PERM_READ_ENCRYPT, read_cs_report, NULL, &current_consumer),
                       BT_GATT_DESCRIPTOR(BT_UUID_HIDS_REPORT_REF, BT_GATT_PERM_READ_ENCRYPT, read_report_ref, NULL, (void *)cs_report_ref),
                       BT_GATT_CCC(NULL, BT_GATT_PERM_READ | BT_GATT_PERM_WRITE),

                       /* Boot Keyboard OUT (for LED feedback) */
                       BT_GATT_CHARACTERISTIC(BT_UUID_HIDS_BOOT_KB_OUT_REPORT, BT_GATT_CHRC_WRITE_WITHOUT_RESP,
                                              BT_GATT_PERM_WRITE_ENCRYPT, NULL, write_boot_kb_out, NULL),
...
static int ble_hid_send_mouse(const hid_mouse_report_t *report)
{
    ...
    LOG_DBG("BLE Mouse notify: btns=%02x dx=%d dy=%d wheel=%d",
            current_mouse.buttons, current_mouse.dx, current_mouse.dy, current_mouse.wheel);
#endif
    return 0;
}
...
static int ble_hid_send_consumer(const hid_consumer_report_t *report)
{
    ...
    LOG_DBG("BLE Consumer notify: usage=0x%04x", current_consumer.usage);
#endif
    return 0;
}
